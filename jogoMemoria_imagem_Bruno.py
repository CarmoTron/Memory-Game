...
def click_na_carta(linha,coluna):
    ...
    if cor == 'black':
        ...
        carta.config(width=CARTAO_SIZE_W,height=CARTAO_SIZE_H)
        carta['image'] = ''
...
def jogada():
    ...
    if carta1['bg'] == carta2['bg']:
        ...
        if carta1 not in cartas_iguais: # Somente redefina a imagem para cartas não correspondentes
            carta1.after(350,lambda c=carta1:definir_imagem(c))
        if carta2 not in cartas_iguais:
            carta2.after(350,lambda c=carta2:definir_imagem(c))
...
######
# imagem 
def definir_imagem(carta):
    caminho_imagem = Image.open("static/verso.png")
    caminho_imagem = caminho_imagem.resize((CARTAO_SIZE_W*10,CARTAO_SIZE_H*20))
    caminho_imagem = ImageTk.PhotoImage(caminho_imagem)
    carta.config(image=caminho_imagem, compound="center",bg='black')
    carta.imagem = caminho_imagem   
###FIM DAS FUNCOES ###
#
#
### MAIN ###
...
for linha in range(N_LIN):
    ...
    for col in range(N_COL):
        #botao
        global carta
        carta = tk.Button(janela, width=CARTAO_SIZE_W*10, height=CARTAO_SIZE_H*20, relief=tk.RAISED, bd=3,command=lambda li=linha, c=col: click_na_carta(li,c))
        #grid é grelha, funcao ja existente 
        definir_imagem(carta)
